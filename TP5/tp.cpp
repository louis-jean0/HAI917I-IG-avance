...
void ICP(std::vector<Vec3> const & ps , std::vector<Vec3> const & nps ,
    ...
    Vec3 centroide_ps(0.0f,0.0f,0.0f);
    Vec3 centroide_qs(0.0f,0.0f,0.0f);
    ...
    for(unsigned int it = 0 ; it < 1 ; ++it) {
        ...
        centroide_ps /= ps.size();
        centroide_qs /= qs.size();

        translation = centroide_qs - centroide_ps;

        std::vector<Vec3> ps_transformed;
        std::vector<Vec3> qs_transformed;

        for(auto& p : ps) {
            ps_transformed.push_back(p - centroide_ps);
        }
        for(const auto& q : qs) {
            qs_transformed.push_back(q - centroide_qs);
        }

        Mat3 covariance_matrix = Mat3::Zero();
        for(const auto& pst : ps_transformed) {
            Vec3 projected_point, projected_normal;
            //HPSS(pst, projected_point, projected_normal, positions, normals, kdtree, KernelType::Gaussian, 0.5);
            unsigned int index = kdtree.nearest(pst);
            //unsigned int index = kdtree.nearest(projected_point);
            covariance_matrix += Mat3::tensor(qs_transformed[index], pst);
        }
        covariance_matrix.setRotation();
        rotation = covariance_matrix;
...
double RMSD(const std::vector<Vec3>& points1, const std::vector<Vec3>& points2) {
    ...
    for (size_t i = 0; i < points1.size(); ++i) {
        Vec3 diff = points1[i] - points2[i];
        sumSquaredDiffs += diff.squareLength(); 
    }

    double meanSquaredDiff = sumSquaredDiffs / points1.size(); 
    return std::sqrt(meanSquaredDiff);
}

double Hausdorff(const std::vector<Vec3>& points1, std::vector<Vec3>& points2) {
    size_t points1_size = points1.size();
    size_t points2_size = points2.size();

    std::vector<double> minDistance(points1_size,std::numeric_limits<double>::max());
    for(size_t i = 0; i < points1_size; i++) {
        for(size_t j = 0; j < points2_size; j++) {
            double distance = Vec3::euclideanDistance(points1[i],points2[j]);
    ...
    minDistance.resize(points2_size,std::numeric_limits<double>::max());
    for(size_t i = 0; i < points2_size; i++) {
        for(size_t j = 0; j < points1_size; j++) {
            double distance = Vec3::euclideanDistance(points2[i],points1[j]);
            if(distance < minDistance[i]) {
                minDistance[i] = distance;
            }
        }
...
int main (int argc, char ** argv) {
    ...
    // ICP :
    {
        ...
        srand(time(NULL));
        Mat3 rotation = Mat3::RandRotation(M_PI / 10); // PLAY WITH THIS PARAMETER !!!!!!
        ...
        normals3 = normals2;
        performICP(20);
        double rmsd = RMSD(positions, positions3);
        std::cout<<"RMSD : "<<rmsd<<std::endl;
        double hausdorff = Hausdorff(positions, positions3);
        std::cout<<"Distance de Hausdorff : "<<hausdorff<<std::endl;
    }
