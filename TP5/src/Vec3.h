...
public:
    ...
    const Vec3 operator - (Vec3 const & other) const {
        return Vec3(mVals[0] - other[0], mVals[1] - other[1], mVals[2] - other[2]);
    }

    static double euclideanDistance(Vec3 const &a, Vec3 const &b) {
        return (a-b).length();
    }
};
